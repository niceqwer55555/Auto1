...
def interfaceTestplan(req):
    ...
    c = csrf(req)
    c.update({'page_list': page_list,'num': num, 'interfaces_all':interfaces_all,'env_paras':env_para,"product_name":product_name,'product_alls':product_all,'intervals':interval})
...
def updateTestplanInterface(request):
    if request.method == "POST":
        raw_data = request.body
        raw_data = json.loads(raw_data)
...
def startInterfaceTestplan(request):
    if request.method == "POST":
        try:
            raw_data = request.body
            raw_data = json.loads(raw_data)
                    ...
                    if (str(public_resp) != '[]'):
                        for rec in public_resp:
                            ...
                            try:
                                ...
                                cache.set(rec.keywords, result)
                                cache.expire(rec.keywords, 3600)
                                print_log('【关联参数】： {' + rec.keywords + '}，匹配的第' + str(
                                    index + 1) + '个值为：' + result)
                            except Exception:
                                error_info = traceback.format_exc()
                                print(error_info)
                                print_log(
                                    '【关联参数】： {' + rec.keywords + '} 在响应数据中未匹配到，请检测前置接口关键字配置，以及Redis是否已启动')
