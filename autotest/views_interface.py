...
def loadModule(request):
    ...
    if AuthUser.objects.filter(username=username).first().is_superuser == 1:
        ...
        product_id = AuthUser.objects.filter(username=username).first().last_name
    items = AutotestplatModule.objects.filter(product_id=product_id).values_list().order_by('id')
    rst = []
    for item in items:
        arr = []
        for j in item:
...
def modModule(request):
    module_id = request.POST.get('id')
    module_name = request.POST.get('module_name')
    AutotestplatModule.objects.filter(id=module_id).update(module_name=module_name)
    return HttpResponse('200')
...
def showAddWindow(req,menu_module_id):
    ...
    menu_module=AutotestplatModule.objects.filter(id=menu_module_id).first()
    if(menu_module):
        menu_module_id=menu_module.module_name
    else:
        menu_module_id='default'
    username = req.session.get('user', '')
    if AuthUser.objects.filter(username=username).first().is_superuser == 1:
...
def showEditInterface(req,edit_id,action):
    ...
    if menu_module:
        menu_module_name=menu_module.module_name
    else:
        menu_module_name='default'
...
def saveEditInterface(req,edit_id):
    if req.method == "POST":
        ...
        menu_module_name = req.POST.get('edit_menu_module_id', '')
        menu_module = AutotestplatModule.objects.filter(module_name=menu_module_name).first()
        if menu_module:
            menu_module_id=menu_module.id
        else:
            menu_module_id=0
        AutotestplatInterfaceTestcase.objects.filter(id=edit_id).update(name=edit_name,url_host=edit_url_host,url=edit_url,body=edit_body,head=edit_head,mode=edit_mode,product_id=edit_product_id,assert_keywords_old=assert_keywords_old,body_format=body_format,menu_module_id=menu_module_id)
...
def saveCopyInterface(req,edit_id):
    if req.method == "POST":
...
def delInterface(request):
    interface_id = request.POST.get('id')
    AutotestplatInterfaceTestcase.objects.filter(id=interface_id).delete()
    AutotestplatParameter.objects.filter(module_id=int(interface_id)).exclude(product_id='testplan').delete()
    return HttpResponse('200')
