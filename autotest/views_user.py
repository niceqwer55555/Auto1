...
def userView(request):
    ...
    return render_to_response("user.html",c)
...
def updateUser(request,userName):
    id = request.POST.get('id')
    cur_id=AuthUser.objects.filter(username=userName).first().id
    user_name = request.POST.get('username')
    user_pwd = request.POST.get('password')
    ...
    return HttpResponse('200,'+str(cur_id))
...
@csrf_exempt
def setUserProduct(request):
    product_name = request.POST.get('product_name')
    user_name = request.session.get('user', '')
    product_id = AutotestplatProduct.objects.filter(product_name=product_name).first().id
    AuthUser.objects.filter(username=user_name).update(last_name=product_id)
    return HttpResponse('200')